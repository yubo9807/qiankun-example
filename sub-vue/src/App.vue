<template>
  <div id="app">
    ...
    <div>
      ...
      <p>全局用户：<code> {{ JSON.stringify(user) }}</code></p>
    </div>
    <div class="btns">
      <template v-if="isInQiankun">
        <button @click="gotoSubReact">跳转到 React 子应用</button>
        <button @click="openSubVue">独立打开 Vue 子应用</button>
      </template>
      <button @click="changeUsername">改变全局用户名称</button>
    </div>
    <router-view />
  </div>
</template>
